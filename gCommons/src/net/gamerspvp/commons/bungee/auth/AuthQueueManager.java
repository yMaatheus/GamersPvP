...
public class AuthQueueManager {
		...
		ProxyServer.getInstance().getScheduler().schedule(instance, new Runnable() {
			...
			public void run() {
				...
				for (int a = 0; a < values.size(); a++) {
					...
					if (server.getOnline() >= maxPlayers || serverInfo == null) { //Verificando se o servidor est§ super lotado.
						continue;
					}
	...
	public void add(ProxiedPlayer proxiedPlayer) throws Exception {
		...
		proxiedPlayer.sendMessage(ChatMessageType.ACTION_BAR, new TextComponent("§aEstamos enviando voc§ at§ o §fSagu§o§a do servidor!"));
	}
	...
	public void remove(ProxiedPlayer proxiedPlayer) {
		...
		proxiedPlayer.sendMessage(ChatMessageType.ACTION_BAR, new TextComponent("§aEnvio ao §fSagu§o §aconcluido com sucesso!"));
	}
