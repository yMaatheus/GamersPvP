	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (args.length > 1) {
			...
			if (args[0].equalsIgnoreCase("normal")) {
				...
				sender.sendMessage(new TextComponent("§aVoc§ definiu o motd normal para: §f" + motd));
				return;
			} else if (args[0].equalsIgnoreCase("maintenance")) {
				NetworkOptions networkOptions = instance.getNetworkOptions();
				networkOptions.setMotdMaintenance(motd);
				networkOptions.publish(instance);
				sender.sendMessage(new TextComponent("§aVoc§ definiu o motd de manuten§§o para: §f" + motd));
				return;
			}
		}
