...
public class GlobalListeners implements Listener {
	...
	public void onProxyPingEvent(ProxyPingEvent event) {
		...
		if (maintenance == null) {
			...
			response.getVersion().setName("§cManuten§§o");
	...
	public void onPreLoginEvent(PreLoginEvent event) {
		if (timestamp > System.currentTimeMillis()) {
			...
			event.setCancelReason(new TextComponent("§cMuitos jogadores est§o tentando conectar no momento, tente novamente!"));
			return;
		}
		...
		if (length < 3 || length > 16 || !name.matches("[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_]+")) {
			event.setCancelled(true);
			event.setCancelReason(new TextComponent("§cSeu nome de usu§rio n§o § permitido no servidor."));
			return;
		}
