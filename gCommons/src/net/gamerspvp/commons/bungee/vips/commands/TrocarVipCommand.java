		...
		if (args.length > 0) {
			try {
				...
				if (playerVip == null || System.currentTimeMillis() > playerVip.getVips().get(groupName)) {
					sender.sendMessage(new TextComponent("§cVoc§ n§o possui §lVIP §cou o tempo foi expirado."));
					return;
				}
				proxiedUserManager.defineUserGroup(playerName, groupName);
				sender.sendMessage(new TextComponent("§aVoc§ trocou seu grupo para §f" + groupName + " §acom sucesso."));
			} catch (Exception e) {
				e.printStackTrace();
			}
			return;
