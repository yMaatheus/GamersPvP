		...
		if (args.length > 0) {
			if (args[0].equalsIgnoreCase("network")) {
				...
				if (args.length >= 4) {
					if (args[1].equalsIgnoreCase("ativar")) {
						...
						try {
							...
							sender.sendMessage(new TextComponent("§cHor§rio inv§lido."));
							return;
						}
						if (time == 0) {
							sender.sendMessage(new TextComponent("§cFormato de hor§rio inv§lido."));
							return;
						}
						String reason = "";
						...
						sender.sendMessage(new TextComponent("§aModo de seguran§a da network foi ativado com sucesso."));
						return;
					}
				}
				if (args[1].equalsIgnoreCase("desativar")) {
					...
					networkOptions.publish(instance);
					sender.sendMessage(new TextComponent("§cModo de seguran§a da network foi desativado com sucesso."));
				}
				return;
			}
			...
			if (!jedis.exists(key)) {
				sender.sendMessage(new TextComponent("§cServidor n§o encontrado."));
				return;
			}
			GameStatus game = instance.getGameStatus(key);
			if (args[1].equalsIgnoreCase("ativar")) {
				game.setStatus(gameStatus.MANUTEN§§O);
				String json = gson.toJson(game);
				jedis.set(key, json);
				jedis.publish("general", "maintenance;" + json);
				sender.sendMessage(new TextComponent("§aModo de seguran§a foi ativado no servidor §f" + args[0] + " §acom sucesso."));
				return;
				...
				jedis.set(key, json);
				jedis.publish("general", "maintenance;" + json);
				sender.sendMessage(new TextComponent("§aModo de seguran§a foi desativado no servidor §f" + args[0]  + " §acom sucesso."));
				return;
			}
		}
