...
public class GladiadorManager {
			...
			public void run() {
				if (gladiador.getAlerts() >= alerts) {
					if ((getClans() < settings.minClans) || (getPlayers() < settings.minPlayers)) {
						Bukkit.broadcastMessage(prefix + "§cO Evento foi cancelado por insufici§ncia de participantes.");
						gladiador.setStatus(statusType.CANCELADO);
						gladiador.teleportAll("saida", "§aVoc§ foi teleportado at§ a saida.", 0, instance);
						gladiador.reset();
						gladiador = null;
						return;
					}
					...
					Bukkit.broadcastMessage(prefix + "§fPremios: §2$§3" + premio + " + §fTags: §6[Gladiador] §3e §5[Mito]");
					Bukkit.broadcastMessage(prefix + "§fKit setado: §3N§o!");
					Bukkit.broadcastMessage(prefix + "§fM§ximo Membros por clan: §3" + settings.getMaxMembersPerClan());
	...
	public void executeCheck() {
		...
		if (clans == 1) {
			...
			if ((System.currentTimeMillis() > gladiador.getTime()) || (clans <= settings.clansForStartDeathmatch)) {
				if (players > 35) {
					System.out.println("[Gladiador] Evento com mais de 35 membros, deathmatch n§o ser§ acionada");
					return;
				}
				executeDeathmatch();
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				gladiador.getParticipantes().keySet().forEach(player -> {
					...
					player.sendMessage("§aVoc§ foi teleportado at§ a Arena Deathmatch.");
				});
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				gladiador.getParticipantes().keySet().forEach(player -> {
					...
					player.sendMessage("§aVoc§ foi teleportado at§ a saida.");
				});
				executeClearDrops();
				gladiador = null;
			}
	...
	public void depositPremio(Player player) {
		player.sendMessage(prefix + "§eVoc§ recebeu §2$§f" + premio + " §ecomo pr§mio.");
