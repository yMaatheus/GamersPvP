...
public class MoneyCommand extends Command {
	...
	public boolean execute(CommandSender sender, String arg, String[] args) {
			...
			if (args.length >= 2) {
				if ((args[0].equalsIgnoreCase("receive")) || (args[0].equalsIgnoreCase("receber"))) {
					...
					if (args[1].equalsIgnoreCase("on")) {
						if (playerMoney.isReciveMoney()) {
							sender.sendMessage("§cO recebimento de dinheiro j§ est§ ativo.");
							return true;
						}
						...
						if (!(playerMoney.isReciveMoney())) {
							sender.sendMessage("§cO recebimento de dinheiro j§ est§ desativado.");
							return true;
						}
				...
				if (args.length >= 3) {
					...
					if (args[0].equalsIgnoreCase("give")) {
						...
						sender.sendMessage("§a[Economia] Voc§ adicionou §f" + balanceAdicioned + " §ade dinheiro para o jogador §f" + targetName);
						...
						sender.sendMessage("§a[Economia] Voc§ removeu §f" + balanceRetired + " §ade dinheiro do jogador §f" + targetName);
						...
						sender.sendMessage("§a[Economia] Voc§ definiu o dinheiro de §f" + targetName + " §apara " + balance);
						...
						if (target.equalsIgnoreCase(player.getName())) {
							player.sendMessage("§cVoc§ n§o pode enviar dinheiro para voc§ mesmo.");
							return false;
						}
						if (!(targetMoney.isReciveMoney())) {
							sender.sendMessage("§cO recebimento de dinheiro do jogador est§ desativado.");
							return false;
						}
						Player targetPlayer = Bukkit.getPlayer(target);
						if (targetPlayer == null) {
							player.sendMessage("§cJogador inv§lido.");
							return false;
						}
						if (playerMoney.getMoney() < value) {
							player.sendMessage("§cVoc§ n§o possui dinheiro suficiente.");
							return false;
						}
						...
						player.sendMessage("§a[Economia] Voc§ enviou §f" + balance + " §ade dinheiro para §f" + targetName);
						targetPlayer.sendMessage("§a[Economia] Voc§ recebeu §f" + balance + " §ade dinheiro do jogador §f" + senderName);
					}
					return true;
				}
		...
		sender.sendMessage("§a[Economia] §7Voc§ possui: " + economyManager.getBalance(playerMoney));
		return true;
	}
	
	public void helpCommands(String arg, CommandSender sender) {
		sender.sendMessage("");
		sender.sendMessage("§a[Economia] Comandos dispon§veis:");
