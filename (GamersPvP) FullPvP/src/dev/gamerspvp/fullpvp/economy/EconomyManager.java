...
public class EconomyManager {
	...
	public void loadData() {
		...
		console.sendMessage("ßa[Economy] Carregando contas...");
		try {
			...
			console.sendMessage("ßa[Economy] Foram carregadas ßf" + i + " ßacontas.");
			rs.close();
			stmt.close();
		} catch (SQLException e) {
			e.printStackTrace();
			console.sendMessage("ßc[Economy] N√O FOI POSSÕVEL CARREGAR AS CONTAS, DESABILITANDO...");
	...
	public void executeUpdateMoneyTop() {
		try {
			...
			if (!(magnataAtual.equalsIgnoreCase(newMagnata))) {
				...
				title.setTitle("ß2" + newMagnata);
				title.setSubtitle("ßa… o novo Magnata!");
				title.broadcast();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String getBalance(PlayerMoney playerMoney) {
		return "ß2$ßf" + format(playerMoney.getMoney());
	}
	
	public String getBalance(double value) {
		return "ß2$ßf" + format(value);
	}
