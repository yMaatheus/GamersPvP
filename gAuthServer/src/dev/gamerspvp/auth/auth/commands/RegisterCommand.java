	...
	public boolean execute(CommandSender sender, String arg, String[] args) {
		...
		if (args.length > 1) {
			...
			if (!(password.equals(confirmPassword))) {
				sender.sendMessage("§cA senha de confirma§§o n§o § igual a outra. Tenha mais aten§§o!");
				return false;
			}
			AuthBukkitManager authManager = instance.getAuthManager();
			if (password.length() < 5) {
				sender.sendMessage("§cA senha escolhida n§o § segura o suficiente.");
				return false;
			}
			if (password.equals("segura") || password.equals("seguraosuficiente")) {
				sender.sendMessage("§cAgora, literalmente, a senha § segura kkkk");
			}
			...
			if (authPlayer.isRegistered()) {
				sender.sendMessage("§cVoc§ j§ est§ registrado. Utilize /logar (senha).");
				return false;
			}
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					player.sendMessage("\n §aCadastro efetuado com sucesso! Seja bem vindo ao servidor, divirta-se e n§o n§s esque§a! \n");
