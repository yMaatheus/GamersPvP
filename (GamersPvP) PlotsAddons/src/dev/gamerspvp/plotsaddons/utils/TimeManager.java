package dev.gamerspvp.plotsaddons.utils;

import java.util.concurrent.TimeUnit;

public class TimeManager {
	
	public static long generateTime(String tipo, int tempo) {
		long Tempo = 0;
		if (tipo.equalsIgnoreCase("segundos")) {
			Tempo = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(tempo);
		}
		if (tipo.equalsIgnoreCase("dias")) {
			Tempo = System.currentTimeMillis() + TimeUnit.DAYS.toMillis(tempo);
		}
		if (tipo.equalsIgnoreCase("horas")) {
			Tempo = System.currentTimeMillis() + TimeUnit.HOURS.toMillis(tempo);
		}
		if (tipo.equalsIgnoreCase("minutos")) {
			Tempo = System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(tempo);
		}
		return Tempo;
	}
