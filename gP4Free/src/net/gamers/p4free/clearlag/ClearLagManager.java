...
public class ClearLagManager {
	...
	public void executeClearDrops(boolean announce) {
		...
		if (announce) {
			Bukkit.broadcastMessage("§c[!] §7Foram removidos §c§l" + cleared + " §7itens do chão.");
		}
		drop = 0;
		closed = false;
	}
	...
	public void addDrops(int drops) {
		...
		if (drop >= 500) {
			this.closed = true;
			Bukkit.broadcastMessage("§c[!] §7O Chão será limpo em §c§l60§7 segundos!");
			new BukkitRunnable() {
				
				@Override
				public void run() {
					
					Bukkit.broadcastMessage("§c[!] §7O Chão será limpo em §c§l30§7 segundos!");
					new BukkitRunnable() {
						
						@Override
						public void run() {
							Bukkit.broadcastMessage("§c[!] §7O Chão será limpo em §c§l10§7 segundos!");
							new BukkitRunnable() {
