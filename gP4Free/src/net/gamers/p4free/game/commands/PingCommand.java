...
public class PingCommand extends Command {
	...
	public boolean execute(CommandSender sender, String arg, String[] args) {
		if (args.length > 0) {
			...
			if (ping > 300) {
				sender.sendMessage("§aO ping de " + target.getName() + " § §4" + ping + "ms");
			} else if (ping > 250) {
				sender.sendMessage("§aO ping de " + target.getName() + " § §c" + ping + "ms");
			} else if (ping > 200) {
				sender.sendMessage("§aO ping de " + target.getName() + " § §e" + ping + "ms");
			} else {
				sender.sendMessage("§aO ping de " + target.getName() + " § §a" + ping + "ms");
			}
			return true;
		}
		...
		if (ping > 300) {
			sender.sendMessage("§aSeu ping § §4" + ping + "ms");
		} else if (ping > 250) {
			sender.sendMessage("§aSeu ping § §c" + ping + "ms");
		} else if (ping > 200) {
			sender.sendMessage("§aSeu ping § §e" + ping + "ms");
		} else {
			sender.sendMessage("§aSeu ping § " + ping + "ms");
		}
		return true;
	}
}