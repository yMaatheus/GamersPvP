...
public class MiniGladiadorManager {
		...
		miniGladiador.setTask(new BukkitRunnable() {
			...
			public void run() {
				if (miniGladiador.getAlerts() >= alerts) {
					...
					Bukkit.broadcastMessage(prefix + "§fTag: §e[MiniGladiador]");
	...
	public void executeCheck() {
		...
		if (clans == 1) {
			...
			if ((System.currentTimeMillis() > miniGladiador.getTime()) || (clans <= settings.clansForStartDeathmatch)) {
				...
				if (players > 35) {
					System.out.println("[MiniGladiador] Evento com mais de 35 membros, deathmatch n§o ser§ acionada");
					return;
				}
				executeDeathmatch();
			}
	...
	private void executeClearDrops() {
		Bukkit.getWorld("P4Free").getEntitiesByClass(org.bukkit.entity.Item.class).forEach(item -> {
			item.remove();
		});
	}
