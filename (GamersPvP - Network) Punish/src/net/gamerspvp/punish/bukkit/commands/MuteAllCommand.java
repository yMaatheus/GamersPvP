...
public class MuteAllCommand extends Command {
	...
	public MuteAllCommand(Main instance) {
		super("muteall");
		instance.registerCommand(this, "muteall", "mutarall");
		this.instance = instance;
	}
	
	@Override
	public boolean execute(CommandSender sender, String arg, String[] args) {
		if (!(sender.hasPermission("punish.muteall"))) {
		...
		if (args.length >= 4) {
			...
			if (target.equalsIgnoreCase(sender.getName())) {
				sender.sendMessage("§cVoc§ n§o pode mutar-se.");
				return false;
			}
			...
			Mute mute = new Mute(target, true, reason, author, time, date);
			new BukkitRunnable() {
					...
					try {
						if (muteTarget != null) {
							if (System.currentTimeMillis() < muteTarget.getTime()) {
								sender.sendMessage("§cEsse jogador j§ encontra-se silenciado.");
								return;
							}
